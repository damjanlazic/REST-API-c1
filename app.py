...
from resources.user import TokenRefresh, User, UserRegister, UserLogin, TokenRefresh, UserLogout
...
def add_claims_to_jwt(identity): # remember identity is what we define when creating an access token (here identity=user_id)
    if identity == 1: # here in real life we should read from database to get a list of administrators
...
def check_if_token_in_blacklist(jwt_headers, jwt_payload):
    return jwt_payload['jti'] in BLACKLIST  # jti means "JWT ID", a unique identifier for a JWT
